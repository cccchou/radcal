...
# Export the uncalibrated probability density if required
    ...
    res_age = np.zeros((int(round(max_res_age - min_res_age + 1)), 1 + dat.shape[0]))
    ...
    for i in range(len(dat)):
        ...
        #print((np.min(res.iloc[:, 0].astype(int)) - min_res_age))
        # res_age[res[:, 0].astype(int) - min_res_age, i + 1] = res[:, 1]

        res_age[round((np.min(temp[f'resage_{i}'].iloc[:, 0]) - min_res_age)): round(np.max(temp[f'resage_{i}'].iloc[:, 0]) - min_res_age+1), i+1] = temp[f'resage_{i}'].iloc[:,1]
...
# Mixture PDF
    if mixture_pdf:
        ...
        ref = f"Mixture_reservoir_age_offset_{'' if f_14R else 'F14R_percent_' if f_14R else 'delta14R_permil_' if delta_14R else 'dcp_percent_'}"
